...
public class AdminSearchService {
    ...
    public ProjectListResponse findDemandAdmin(String search, Pageable pageable, String univ) {
        User user = userRepository.findByEmail(SecurityUtil.getLoginUsername())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST));
        ...
        findDemandProjects = demandProjectRepository.findByAdminFormSearch(currentDate, search, univ, pageable);

        Page<ProjectInfo> projectDtos = findDemandProjects.map(demandProject -> new ProjectInfo(demandProject));
